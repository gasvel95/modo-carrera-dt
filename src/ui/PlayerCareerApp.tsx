...
import { createPlayerCareer, finishPlayerSeason, playPlayerBlock, playerRetirementAge, retirePlayer, setTrainingFocus, startPlayerSeason } from "../game-engine/playerCareerEngine";
import type { PlayerCareerState, PlayerPosition, PreferredFoot, TrainingFocus } from "../domain/playerCareer";
...
type PlayerScreen = "setup" | "offers" | "season" | "summary" | "history" | "arrival" | "retire" | "retired";

function ClubBadge({ shortName }: { shortName: string }) {
  return <span className={styles.clubBadge} aria-hidden="true">{shortName.slice(0, 3)}</span>;
}
...
export function PlayerCareerApp({ onExit }: { onExit: () => void }) {
  ...
  useEffect(() => {
    ...
    try {
      ...
      setState(parsed);
      setScreen(parsed.retirement ? "retired" : parsed.season ? "season" : parsed.history.length ? "offers" : "offers");
  ...
  const play = () => {
    ...
    setBusy(true);
    window.setTimeout(() => { setState(playPlayerBlock(state)); setBusy(false); }, 500);
  };
  ...
  return <main className={`game-shell ${styles.shell}`}>
    ...
    {state && !state.retirement && <nav className={styles.playerNav} aria-label="Carrera de jugador">
      <button onClick={() => setScreen(state.season ? "season" : "offers")}>CARRERA</button>
    ...
    {screen === "offers" && state && <section className={styles.content}>
      ...
      <div className={styles.offerGrid}>{state.offers.map((offer) => <article key={offer.club.id}><div className={styles.offerTop}><span>{offer.kind === "renewal" ? "RENOVACIÓN" : "OFERTA"}</span><b>{offer.club.division}</b></div><ClubBadge shortName={offer.club.shortName} /><h3>{offer.club.name}</h3><small>{offer.club.region}</small><p>{offer.reason}</p><dl><div><dt>PLANTEL</dt><dd>{offer.club.strength}</dd></div><div><dt>TU MEDIA</dt><dd>{state.player.overall}</dd></div></dl><button onClick={() => sign(offer.club.id)}>{offer.kind === "renewal" ? "RENOVAR" : "FIRMAR CONTRATO"} →</button></article>)}</div>
    </section>}

    {screen === "arrival" && state?.season && <section className={styles.arrival}>
      <p>HITO DE CARRERA · {state.season.year}</p><h1>LLEGASTE<br />A PRIMERA.</h1><ClubBadge shortName={state.season.club.shortName} /><h2>{state.season.club.name}</h2><span>El objetivo se cumplió, pero tu carrera no terminó. Ahora podés jugar en Primera, cambiar de club, sumar temporadas y decidir cuándo colgar los botines.</span><button className={styles.primary} onClick={() => setScreen("season")}>JUGAR EN PRIMERA →</button>
    </section>}
    ...
    {screen === "season" && state?.season && <section className={styles.content}>
      ...
      <div className={styles.seasonHead}><div><ClubBadge shortName={state.season.club.shortName} /><span><small>TEMPORADA {state.season.year} · {state.season.club.division}</small><strong>{state.season.club.name}</strong><b>{state.player.name} · {state.player.position} · {state.player.age} AÑOS</b></span></div><div><small>MEDIA</small><strong>{state.player.overall}</strong><b>POT. {state.player.potential}</b></div></div>
